...
namespace UPRLuaProfiler
{
    ...
    {
        private static byte[] MagicTag = { 26, 8, 110, 123, 187, 39, 7, 0 };
        ...
        public static bool enableDeepLua = false;
        public static bool enableDeepMono = false;
       ...
       // private static int m_frameCount = 0;
        ...
        public static void ConnectServer(string host, int port)
        {
            ...
            m_serverThread = new Thread(new ThreadStart(ListenTcpPort));
            m_serverThread.Start();
        }


        public static void ListenTcpPort()
        {
            if (m_client != null) return;
            port = GetAvailablePort(port, 1, "tcp");
            IPAddress myIP = IPAddress.Parse(host);
            _tcpListener = new TcpListener(myIP, port);
            _tcpListener.Start();
           // UnityEngine.Debug.LogError("Start Listening IP: " + host + " on port: " + port);
           keepListen = true;
            while (keepListen)
            {
                ...
                try
                {
                    ...
                    if (m_client != null)
                    {
                        //UnityEngine.Debug.LogError("<color=#00ff00>Lua Connect Success</color>");
                ...
                {
                    UnityEngine.Debug.Log("Listening error:" + e.ToString());
                    Close();
                }
                Thread.Sleep(1000);
            }
        ...
        #endregion
        public static int GetAvailablePort(int beginPort, int maxIter, string type)
        {
            ...
            for (int port = beginPort; port < beginPort + maxIter; port++)
            {
                ...
                {
                    UnityEngine.Debug.Log("Port not available " + port);
                }
        ...
        private static void DoSendMessage()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    if (m_sendThread == null)
                    {
                        UnityEngine.Debug.Log("<color=#ff0000>lua m_sendThread null</color>");
                        return;
                    }
                    if (m_sampleQueue.Count > 0)
                    {
                        ...
                        {
                            NetBase s = null;
                            // header type length serialize  end
                            lock (m_sampleQueue)
                            {
                            ...
                            {
                               // UnityEngine.Debug.LogError("send message 1");
                                msbw.Write((int)1);
                            }
...
#pragma warning restore 0168
        ...
        private static void DoRecieveMessage()
        {
            while (true)
            {
                try
                {
                    if (m_receiveThread == null)
                    {
                        UnityEngine.Debug.Log("<color=#ff0000>lua m_receiveThread null</color>");
                        return;
                    }
                    if (ns.CanRead && ns.DataAvailable)
                    {
                        string resultMess = ParseMessage(br);
                        if (resultMess.Contains("Profiler"))
                        {
                            string[] sess = resultMess.Split(':');
                            if (sess.Length == 3)
                            {
                                enableDeepLua = Convert.ToBoolean(sess[1]);
                                enableDeepMono = Convert.ToBoolean(sess[2]);
                            }
                        }
                    }
                }
        ...
        private static string ParseMessage(BinaryReader binaryReader)
        {
            ...
            {
                UnityEngine.Debug.Log("ParseMessage Error " + e);
            }
            return result;
        }
    }
